...
const bentoItems: BentoItem[] = [
	...
	},
	{
		...
		description: "Working with the leading AI and cloud providers to deliver cutting-edge solutions",
		icons: true,
		href: "#",
		feature: "icons",
		size: "md",
...
const CounterAnimation = ({ start, end, suffix = "" }: { start: number; end: number; suffix?: string }) => {
		...
		<div className="mt-3 relative">
			...
			{timeline.map((item) => (
				...
				>
					...
					<div>
						<div className="text-sm font-medium text-neutral-900 dark:text-neutral-100">{item.year}</div>
...
const BentoCard = ({ item }: { item: BentoItem }) => {
	...
	return (
		...
		>
			...
			>
				<div className="relative z-10 flex flex-col gap-3 h-full" style={{ transform: "translateZ(20px)" }}>
					<div className="space-y-2 flex-1 flex flex-col">
						...
						{item.feature === "timeline" && item.timeline && <TimelineFeature timeline={item.timeline} />}

						{item.feature === "icons" && <IconsFeature />}

						{item.feature === "typing" && item.typingText && <TypingCodeFeature text={item.typingText} />}
...
export default function BentoGrid() {
	return (
		<section className="relative py-24 sm:py-32 bg-white dark:bg-black overflow-hidden">
			<div
				className="max-w-6xl
		 px-6 mx-auto px-4 sm:px-6 lg:px-8"
			>
				{/* Bento Grid */}
				<motion.div
					initial="hidden"
