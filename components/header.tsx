...
export default function Header() {
	...
	return (
		<>
			...
			>
				<div className="size-full max-w-7xl px-6 mx-auto standalone:pt-16 flex justify-between bg-background-grainy">
					<Link
						aria-label="home"
						href="/"
					...
					<nav className="hidden md:block">
						<ul
							className={`grid grid-cols-5 gap-4 text-center h-full`}
						>
					...
					>
						<svg
							...
							stroke="currentColor"
							strokeWidth="2"
							className="lucide lucide-menu size-8 text-foreground/50 relative"
							aria-hidden="true"
						>
							<path
								className={`transition duration-300 origin-center ${open ? "-rotate-45 absolute top-1/2 left-1/2 translate-x-1.25 translate-y-1.25" : "rotate-0"}`}
								d="M4 5h16"
							></path>
							<path
								className={`transition duration-300 ${open ? "opacity-0" : "opacity-100"}`}
								d="M4 12h16"
							></path>
							<path
								className={`transition duration-300 origin-center ${open ? "rotate-45 absolute top-1/2 left-1/2 translate-x-1.25 -translate-y-1.25" : "rotate-0"}`}
			...
			<div aria-hidden="true" className="h-0 standalone:h-30" />
		</>
	);
}
